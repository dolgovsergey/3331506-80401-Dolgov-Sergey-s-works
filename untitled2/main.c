...
double y_step_size = 0.2;
double x_step_size = 0.2;
double cutting_depth  = 2;
double x_sp = 0;
double y_sp = -40;
...
int main() {
    ...
    main_arr[0][0] = -12;//X0 relative
    main_arr[0][1] = -12;// Y0 relative
    main_arr[0][2] = -38;// offset Y




///                   0123456789 123456789 12345678
    fill_main_arr("l32.1a188r20l21.5a252l6.6a180l35.6a90l4a180l5a102e");
    ...
    for (int i = 0; i < 15; i++) {
        printf("line %d ", i);
        for (int j = 0; j < 5; j++) {
            printf("%f ", main_arr[i][j]);
        }
        printf(" types arr value %d", types_arr[i]);
        printf(" %f ", ip_arr[i]);
        printf("\n");
    }
   ...
   current_pos[1]=-79;
  current_pos[2]=-39;
sector_left_cutting(-13,-60);
printf("\nstarting \n");
final_pass(-13,-60);

printf("\n%f",current_func_value_count(-46.8));

    //printf("\nradius check\n");
    double y_check = 2 * sin(acos(0.5));
   // printf("\n y check = %f", y_check);
    return 0;
}
